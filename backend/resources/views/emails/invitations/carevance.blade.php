{{--
    ...
    the organisation and usually none at all with '.config('brand.label').', so the org name
    leads the eyebrow, the headline and the subject. {{ config('brand.label') }} appears once, as
...
<x-mail.layout
    ...
    :footerLead="'<strong style=\'color:#16191C;\'>'.config('brand.label').'</strong> — the HR &amp; payroll platform '.e($organizationName).' runs on.'"
...
>
    ...
    <p style="margin:0 0 20px;font-size:15px;line-height:1.7;color:#4E565D;">
        {{ $organizationName }} runs its HR and payroll on {{ config('brand.label') }}. Set a password and your account is ready to use.
    </p>
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:1.7;color:#4E565D;">
        <tr>
            ...
            <td style="padding:3px 0;">Set your password on the {{ config('brand.label') }} sign-in page.</td>
        </tr>
        <tr>
